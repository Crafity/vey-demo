(function (global, factory) {
	...
	(factory((global.vendor = global.vendor || {})));
...
assign(CallbackQueue.prototype, {
  ...
  notifyAll: function () {
    ...
    if (callbacks) {
      !(callbacks.length === contexts.length) ? invariant(false, 'Mismatched list of contexts in callback queue') : undefined;
      this._callbacks = null;
      this._contexts = null;
  ...
  processUpdates: function (updates, markupList) {
    ...
    for (var i = 0; i < updates.length; i++) {
      ...
      if (update.type === ReactMultiChildUpdateTypes.MOVE_EXISTING || update.type === ReactMultiChildUpdateTypes.REMOVE_NODE) {
        ...
        !updatedChild ? invariant(false, 'processUpdates(): Unable to find child %s of element. This ' + 'probably means the DOM was unexpectedly mutated (e.g., by the ' + 'browser), usually due to forgetting a <tbody> when using tables, ' + 'nesting tags like <form>, <p>, or <a>, or using non-SVG elements ' + 'in an <svg> parent. Try inspecting the child nodes of the element ' + 'with React ID `%s`.', updatedIndex, parentID) : undefined;
...
var DOMPropertyInjection = {
  ...
  injectDOMPropertyConfig: function (domPropertyConfig) {
    ...
    for (var propName in Properties) {
      ...
      !(!propertyInfo.mustUseAttribute || !propertyInfo.mustUseProperty) ? invariant(false, 'DOMProperty: Cannot require using both attribute and property: %s', propName) : undefined;
      !(propertyInfo.mustUseProperty || !propertyInfo.hasSideEffects) ? invariant(false, 'DOMProperty: Properties that have side effects must use property: %s', propName) : undefined;
      !(propertyInfo.hasBooleanValue + propertyInfo.hasNumericValue + propertyInfo.hasOverloadedBooleanValue <= 1) ? invariant(false, 'DOMProperty: Value can be one of boolean, overloaded boolean, or ' + 'numeric value, but not a combination: %s', propName) : undefined;
...
var Danger = {
  ...
  dangerouslyRenderMarkup: function (markupList) {
    !ExecutionEnvironment.canUseDOM ? invariant(false, 'dangerouslyRenderMarkup(...): Cannot render markup in a worker ' + 'thread. Make sure `window` and `document` are available globally ' + 'before requiring React when unit testing or use ' + 'ReactDOMServer.renderToString for server rendering.') : undefined;
    var nodeName;
    var markupByNodeName = {};
    // Group markup by `nodeName` if a wrap is necessary, else by '*'.
    for (var i = 0; i < markupList.length; i++) {
      !markupList[i] ? invariant(false, 'dangerouslyRenderMarkup(...): Missing markup.') : undefined;
    ...
    for (nodeName in markupByNodeName) {
      ...
      for (var j = 0; j < renderNodes.length; ++j) {
        ...
        if (renderNode.hasAttribute && renderNode.hasAttribute(RESULT_INDEX_ATTR)) {
          ...
          !!resultList.hasOwnProperty(resultIndex) ? invariant(false, 'Danger: Assigning to an already-occupied result index.') : undefined;
    ...
    // array.
    !(resultListAssignmentCount === resultList.length) ? invariant(false, 'Danger: Did not assign to every index of resultList.') : undefined;

    !(resultList.length === markupList.length) ? invariant(false, 'Danger: Expected markup to render %s nodes, but rendered %s.', markupList.length, resultList.length) : undefined;
  ...
  dangerouslyReplaceNodeWithMarkup: function (oldChild, markup) {
    !ExecutionEnvironment.canUseDOM ? invariant(false, 'dangerouslyReplaceNodeWithMarkup(...): Cannot render markup in a ' + 'worker thread. Make sure `window` and `document` are available ' + 'globally before requiring React when unit testing or use ' + 'ReactDOMServer.renderToString() for server rendering.') : undefined;
    !markup ? invariant(false, 'dangerouslyReplaceNodeWithMarkup(...): Missing markup.') : undefined;
    !(oldChild.tagName.toLowerCase() !== 'html') ? invariant(false, 'dangerouslyReplaceNodeWithMarkup(...): Cannot replace markup of the ' + '<html> node. This is because browser quirks make this unreliable ' + 'and/or slow. If you want to render to the root you must use ' + 'server rendering. See ReactDOMServer.renderToString().') : undefined;
...
var EventPluginHub = {
  ...
  putListener: function (id, registrationName, listener) {
    !(typeof listener === 'function') ? invariant(false, 'Expected %s listener to be a function, instead got type %s', registrationName, typeof listener) : undefined;
...
function publishEventForPlugin(dispatchConfig, PluginModule, eventName) {
  !!EventPluginRegistry.eventNameDispatchConfigs.hasOwnProperty(eventName) ? invariant(false, 'EventPluginHub: More than one plugin attempted to publish the same ' + 'event name, `%s`.', eventName) : undefined;
...
function publishRegistrationName(registrationName, PluginModule, eventName) {
  !!EventPluginRegistry.registrationNameModules[registrationName] ? invariant(false, 'EventPluginHub: More than one plugin attempted to publish the same ' + 'registration name, `%s`.', registrationName) : undefined;
...
var injection = {
  ...
  injectMount: function (InjectedMount) {
    ...
    {
      warning(InjectedMount && InjectedMount.getNode && InjectedMount.getID, 'EventPluginUtils.injection.injectMount(...): Injected Mount ' + 'module is missing getNode or getID.');
    }
  }
};
...
function executeDirectDispatch(event) {
  ...
  !!Array.isArray(dispatchListener) ? invariant(false, 'executeDirectDispatch(...): Invalid `event`.') : undefined;
...
function _assertSingleLink(inputProps) {
  !(inputProps.checkedLink == null || inputProps.valueLink == null) ? invariant(false, 'Cannot provide a checkedLink and a valueLink. If you want to use ' + 'checkedLink, you probably don\'t want to use valueLink and vice versa.') : undefined;
}
function _assertValueLink(inputProps) {
  _assertSingleLink(inputProps);
  !(inputProps.value == null && inputProps.onChange == null) ? invariant(false, 'Cannot provide a valueLink and a value or onChange event. If you want ' + 'to use value or onChange, you probably don\'t want to use valueLink.') : undefined;
}

function _assertCheckedLink(inputProps) {
  _assertSingleLink(inputProps);
  !(inputProps.checked == null && inputProps.onChange == null) ? invariant(false, 'Cannot provide a checkedLink and a checked property or onChange event. ' + 'If you want to use checked or onChange, you probably don\'t want to ' + 'use checkedLink') : undefined;
}
...
function assign(target, sources) {
  ...
  for (var nextIndex = 1; nextIndex < arguments.length; nextIndex++) {
    var nextSource = arguments$1[nextIndex];
    if (nextSource == null) {
      continue;
    }
...
var standardReleaser = function (instance) {
  var Klass = this;
  !(instance instanceof Klass) ? invariant(false, 'Trying to release an instance into a pool of a different type.') : undefined;
  instance.destructor();
...
var ReactBrowserComponentMixin = {
   ...
   */
  getDOMNode: function () {
    warning(this.constructor[didWarnKey], '%s.getDOMNode(...) is deprecated. Please use ' + 'ReactDOM.findDOMNode(instance) instead.', ReactInstanceMap.get(this).getName() || this.tagName || 'Unknown');
...
function warnSetProps() {
  if (!warnedSetProps) {
    warnedSetProps = true;
    warning(false, 'setProps(...) and replaceProps(...) are deprecated. ' + 'Instead, call render again at the top level.');
  }
}
...
function validateMethodOverride(proto, name) {
  ...
  if (ReactClassMixin.hasOwnProperty(name)) {
    !(specPolicy === SpecPolicy.OVERRIDE_BASE) ? invariant(false, 'ReactClassInterface: You are attempting to override ' + '`%s` from your class specification. Ensure that your method names ' + 'do not overlap with React methods.', name) : undefined;
  }

  // Disallow defining methods more than once unless explicitly allowed.
  if (proto.hasOwnProperty(name)) {
    !(specPolicy === SpecPolicy.DEFINE_MANY || specPolicy === SpecPolicy.DEFINE_MANY_MERGED) ? invariant(false, 'ReactClassInterface: You are attempting to define ' + '`%s` on your component more than once. This conflict may be due ' + 'to a mixin.', name) : undefined;
  }
}
...
function mixSpecIntoComponent(Constructor, spec) {
  ...
  !(typeof spec !== 'function') ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a component class as a mixin. Instead, just use a regular object.') : undefined;
  !!ReactElement.isValidElement(spec) ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a component as a mixin. Instead, just use a regular object.') : undefined;
    ...
    if (RESERVED_SPEC_KEYS.hasOwnProperty(name)) {
      ...
      if (shouldAutoBind) {
        ...
        if (isAlreadyDefined) {
          ...
          !(isReactClassMethod && (specPolicy === SpecPolicy.DEFINE_MANY_MERGED || specPolicy === SpecPolicy.DEFINE_MANY)) ? invariant(false, 'ReactClass: Unexpected spec policy %s for key %s ' + 'when mixing in component specs.', specPolicy, name) : undefined;
...
function mixStaticSpecIntoComponent(Constructor, statics) {
  ...
  for (var name in statics) {
    ...
    !!isReserved ? invariant(false, 'ReactClass: You are attempting to define a reserved ' + 'property, `%s`, that shouldn\'t be on the "statics" key. Define it ' + 'as an instance property instead; it will still be accessible on the ' + 'constructor.', name) : undefined;

    var isInherited = (name in Constructor);
    !!isInherited ? invariant(false, 'ReactClass: You are attempting to define ' + '`%s` on your component more than once. This conflict may be ' + 'due to a mixin.', name) : undefined;
...
function mergeIntoWithNoDuplicateKeys(one, two) {
  !(one && two && typeof one === 'object' && typeof two === 'object') ? invariant(false, 'mergeIntoWithNoDuplicateKeys(): Cannot merge non-objects.') : undefined;

  for (var key in two) {
    if (two.hasOwnProperty(key)) {
      !(one[key] === undefined) ? invariant(false, 'mergeIntoWithNoDuplicateKeys(): ' + 'Tried to merge two objects with the same key: `%s`. This conflict ' + 'may be due to a mixin; in particular, this may be caused by two ' + 'getInitialState() or getDefaultProps() methods returning objects ' + 'with clashing keys.', key) : undefined;
      one[key] = two[key];
    }
  }
  return one;
...
var ReactClass = {
  ...
  createClass: function (spec) {
    var Constructor = function (props, context, updater) {
      ...
      !(typeof initialState === 'object' && !Array.isArray(initialState)) ? invariant(false, '%s.getInitialState(): must return an object or null', Constructor.displayName || 'ReactCompositeComponent') : undefined;
    ...
    !Constructor.prototype.render ? invariant(false, 'createClass(...): Class specification must implement a `render` method.') : undefined;
...
var ReactComponentEnvironment = {
  ...
  injection: {
    injectEnvironment: function (environment) {
      !!injected ? invariant(false, 'ReactCompositeComponent: injectEnvironment() can only be called once.') : undefined;
...
var ReactCompositeComponentMixin = {
  ...
  mountComponent: function (rootID, transaction, context) {
    ...
    !(typeof initialState === 'object' && !Array.isArray(initialState)) ? invariant(false, '%s.state: must be set to an object or null', this.getName() || 'ReactCompositeComponent') : undefined;
  ...
  _processChildContext: function (currentContext) {
    ...
    if (childContext) {
      !(typeof Component.childContextTypes === 'object') ? invariant(false, '%s.getChildContext(): childContextTypes must be defined in order to ' + 'use getChildContext().', this.getName() || 'ReactCompositeComponent') : undefined;
      {
        this._checkPropTypes(Component.childContextTypes, childContext, ReactPropTypeLocations.childContext);
      }
      for (var name in childContext) {
        !(name in Component.childContextTypes) ? invariant(false, '%s.getChildContext(): key "%s" is not defined in childContextTypes.', this$1.getName() || 'ReactCompositeComponent', name) : undefined;
      }
    ...
    for (var propName in propTypes) {
      if (propTypes.hasOwnProperty(propName)) {
        ...
        try {
          ...
          !(typeof propTypes[propName] === 'function') ? invariant(false, '%s: %s type `%s` is invalid; it must be a function, usually ' + 'from React.PropTypes.', componentName || 'React class', ReactPropTypeLocationNames[location], propName) : undefined;
        ...
        if (error instanceof Error) {
          ...
          var addendum = getDeclarationErrorAddendum(this$1);
  ...
  _renderValidatedComponent: function () {
    ...
    renderedComponent === null || renderedComponent === false || ReactElement.isValidElement(renderedComponent)) ? invariant(false, '%s.render(): A valid ReactComponent must be returned. You may have ' + 'returned undefined, an array or some other invalid object.', this.getName() || 'ReactCompositeComponent') : undefined;
    return renderedComponent;
  },
  ...
  attachRef: function (ref, component) {
    ...
    !(inst != null) ? invariant(false, 'Stateless function components cannot have refs.') : undefined;
...
function legacyIsMounted() {
  var component = this._reactInternalComponent;
  {
    warning(false, 'ReactDOMComponent: Do not access .isMounted() of a DOM node.%s', getDeclarationErrorAddendum(component));
  }
  return !!component;
}
...
function legacySetStateEtc() {
  {
    var component = this._reactInternalComponent;
    warning(false, 'ReactDOMComponent: Do not access .setState(), .replaceState(), or ' + '.forceUpdate() of a DOM node. This is a no-op.%s', getDeclarationErrorAddendum(component));
  }
}
...
function legacySetProps(partialProps, callback) {
  var component = this._reactInternalComponent;
  {
    warning(false, 'ReactDOMComponent: Do not access .setProps() of a DOM node. ' + 'Instead, call ReactDOM.render again at the top level.%s', getDeclarationErrorAddendum(component));
  }
  if (!component) {
    return;
  }
...
function legacyReplaceProps(partialProps, callback) {
  var component = this._reactInternalComponent;
  {
    warning(false, 'ReactDOMComponent: Do not access .replaceProps() of a DOM node. ' + 'Instead, call ReactDOM.render again at the top level.%s', getDeclarationErrorAddendum(component));
  }
  if (!component) {
    return;
  }
...
function assertValidProps(component, props) {
  ...
  if (props.dangerouslySetInnerHTML != null) {
    ...
    !(typeof props.dangerouslySetInnerHTML === 'object' && HTML in props.dangerouslySetInnerHTML) ? invariant(false, '`props.dangerouslySetInnerHTML` must be in the form `{__html: ...}`. ' + 'Please visit https://fb.me/react-invariant-dangerously-set-inner-html ' + 'for more information.') : undefined;
  }
  {
  ...
  !(props.style == null || typeof props.style === 'object') ? invariant(false, 'The `style` prop expects a mapping from style properties to values, ' + 'not a string. For example, style={{marginRight: spacing + \'em\'}} when ' + 'using JSX.%s', getDeclarationErrorAddendum(component)) : undefined;
}
...
function trapBubbledEventsLocal() {
  ...
  !inst._rootNodeID ? invariant(false, 'Must be mounted to trap events') : undefined;
  var node = ReactMount.getNode(inst._rootNodeID);
  !node ? invariant(false, 'trapBubbledEvent(...): Requires node to be rendered.') : undefined;
...
function validateDangerousTag(tag) {
  if (!hasOwnProperty.call(validatedTagCache, tag)) {
    !VALID_TAG_REGEX.test(tag) ? invariant(false, 'Invalid tag: %s', tag) : undefined;
...
ReactDOMComponent.Mixin = {
  ...
  _createOpenTagMarkupAndPutListeners: function (transaction, props) {
    ...
    for (var propKey in props) {
      ...
      if (registrationNameModules.hasOwnProperty(propKey)) {
        if (propValue) {
          enqueuePutListener(this$1._rootNodeID, propKey, propValue, transaction);
        }
      } else {
        if (propKey === STYLE) {
          if (propValue) {
            {
              ...
              this$1._previousStyle = propValue;
            }
            propValue = this$1._previousStyleCopy = assign({}, props.style);
          }
          propValue = CSSPropertyOperations.createMarkupForStyles(propValue);
        }
        var markup = null;
        if (this$1._tag != null && isCustomComponent(this$1._tag, props)) {
          if (propKey !== CHILDREN) {
  ...
  _updateDOMProperties: function (lastProps, nextProps, transaction, node) {
    ...
    for (propKey in lastProps) {
      ...
      if (propKey === STYLE) {
        var lastStyle = this$1._previousStyleCopy;
        ...
        this$1._previousStyleCopy = null;
      } else if (registrationNameModules.hasOwnProperty(propKey)) {
        if (lastProps[propKey]) {
          // Only call deleteListener if there was a listener previously or
          // else willDeleteListener gets called when there wasn't actually a
          // listener (e.g., onClick={null})
          deleteListener(this$1._rootNodeID, propKey);
        }
      } else if (DOMProperty.properties[propKey] || DOMProperty.isCustomAttribute(propKey)) {
        if (!node) {
          node = ReactMount.getNode(this$1._rootNodeID);
        }
    ...
    for (propKey in nextProps) {
      ...
      var lastProp = propKey === STYLE ? this$1._previousStyleCopy : lastProps[propKey];
      ...
      if (propKey === STYLE) {
        if (nextProp) {
          {
            checkAndWarnForMutatedStyle(this$1._previousStyleCopy, this$1._previousStyle, this$1);
            this$1._previousStyle = nextProp;
          }
          nextProp = this$1._previousStyleCopy = assign({}, nextProp);
        } else {
          this$1._previousStyleCopy = null;
        }
        if (lastProp) {
        ...
        if (nextProp) {
          enqueuePutListener(this$1._rootNodeID, propKey, nextProp, transaction);
        } else if (lastProp) {
          deleteListener(this$1._rootNodeID, propKey);
        }
      } else if (isCustomComponent(this$1._tag, nextProps)) {
        if (!node) {
          node = ReactMount.getNode(this$1._rootNodeID);
        }
        if (propKey === CHILDREN) {
...
var ReactDOMIDOperations = {
  ...
  updatePropertyByID: function (id, name, value) {
    ...
    !!INVALID_PROPERTY_ERRORS.hasOwnProperty(name) ? invariant(false, 'updatePropertyByID(...): %s', INVALID_PROPERTY_ERRORS[name]) : undefined;
...
function _handleChange(event) {
  ...
  if (props.type === 'radio' && name != null) {
    ...
    for (var i = 0; i < group.length; i++) {
      ...
      !otherID ? invariant(false, 'ReactDOMInput: Mixing React and non-React radio inputs with the ' + 'same `name` is not supported.') : undefined;
      var otherInstance = instancesByReactID[otherID];
      !otherInstance ? invariant(false, 'ReactDOMInput: Unknown radio button ID %s.', otherID) : undefined;
...
function getNextDescendantID(ancestorID, destinationID) {
  !(isValidID(ancestorID) && isValidID(destinationID)) ? invariant(false, 'getNextDescendantID(%s, %s): Received an invalid React DOM ID.', ancestorID, destinationID) : undefined;
  !isAncestorIDOf(ancestorID, destinationID) ? invariant(false, 'getNextDescendantID(...): React has made an invalid assumption about ' + 'the DOM hierarchy. Expected `%s` to be an ancestor of `%s`.', ancestorID, destinationID) : undefined;
...
function getFirstCommonAncestorID(oneID, twoID) {
  ...
  !isValidID(longestCommonID) ? invariant(false, 'getFirstCommonAncestorID(%s, %s): Expected a valid React DOM ID: %s', oneID, twoID, longestCommonID) : undefined;
...
function traverseParentPath(start, stop, cb, arg, skipFirst, skipLast) {
  start = start || '';
  stop = stop || '';
  !(start !== stop) ? invariant(false, 'traverseParentPath(...): Cannot traverse from and to the same ID, `%s`.', start) : undefined;
  var traverseUp = isAncestorIDOf(stop, start);
  !(traverseUp || isAncestorIDOf(start, stop)) ? invariant(false, 'traverseParentPath(%s, %s, ...): Cannot traverse from two IDs that do ' + 'not have a parent path.', start, stop) : undefined;
  ...
  for (var id = start;; /* until break */id = traverse(id, stop)) {
    ...
    !(depth++ < MAX_TREE_DEPTH) ? invariant(false, 'traverseParentPath(%s, %s, ...): Detected an infinite loop while ' + 'traversing the React DOM ID tree. This may be due to malformed IDs: %s', start, stop, id) : undefined;
  }
}
...
function getID(node) {
  ...
  if (id) {
    if (nodeCache.hasOwnProperty(id)) {
      ...
      if (cached !== node) {
        !!isValid(cached, id) ? invariant(false, 'ReactMount: Two valid but unequal nodes with the same `%s`: %s', ATTR_NAME, id) : undefined;
...
function isValid(node, id) {
  if (node) {
    !(internalGetID(node) === id) ? invariant(false, 'ReactMount: Unexpected modification of `%s`', ATTR_NAME) : undefined;
...
var ReactMount = {
  ...
  _registerComponent: function (nextComponent, container) {
    !(container && (container.nodeType === ELEMENT_NODE_TYPE || container.nodeType === DOC_NODE_TYPE || container.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE)) ? invariant(false, '_registerComponent(...): Target container is not a DOM element.') : undefined;
  ...
  renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    !(parentComponent != null && parentComponent._reactInternalInstance != null) ? invariant(false, 'parentComponent must be a valid React Component') : undefined;
    return ReactMount._renderSubtreeIntoContainer(parentComponent, nextElement, container, callback);
  },

  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    !ReactElement.isValidElement(nextElement) ? invariant(false, 'ReactDOM.render(): Invalid component element.%s', typeof nextElement === 'string' ? ' Instead of passing an element string, make sure to instantiate ' + 'it by passing it to React.createElement.' : typeof nextElement === 'function' ? ' Instead of passing a component class, make sure to instantiate ' + 'it by passing it to React.createElement.' :
    // Check if it quacks like an element
    nextElement != null && nextElement.props !== undefined ? ' This may be caused by unintentionally loading two independent ' + 'copies of React.' : '') : undefined;
  ...
  unmountComponentAtNode: function (container) {
    ...
    !(container && (container.nodeType === ELEMENT_NODE_TYPE || container.nodeType === DOC_NODE_TYPE || container.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE)) ? invariant(false, 'unmountComponentAtNode(...): Target container is not a DOM element.') : undefined;
...

    {
      ...
      if (rootElement && rootElement.parentNode !== container) {
        ...
        if (containerChild && reactRootID === internalGetID(containerChild)) {
          ...
          warning(false, 'ReactMount: Root element has been removed from its original ' + 'container. New container: %s', rootElement.parentNode);
        }
      }
    }
  ...
  findComponentRoot: function (ancestorNode, targetID) {
...

    {
      ...
      warning(deepestAncestor != null, 'React can\'t find the root component node for data-reactid value ' + '`%s`. If you\'re seeing this message, it probably means that ' + 'you\'ve loaded two copies of React on the page. At this time, only ' + 'a single copy of React can be loaded at a time.', targetID);
    }
  ...
  _mountImageIntoNode: function (markup, container, shouldReuseMarkup, transaction) {
    !(container && (container.nodeType === ELEMENT_NODE_TYPE || container.nodeType === DOC_NODE_TYPE || container.nodeType === DOCUMENT_FRAGMENT_NODE_TYPE)) ? invariant(false, 'mountComponentIntoNode(...): Target container is not valid.') : undefined;
    ...
    !(container.nodeType !== DOC_NODE_TYPE) ? invariant(false, 'You\'re trying to render a component to the document but ' + 'you didn\'t use server rendering. We can\'t do this ' + 'without using server rendering due to cross-browser quirks. ' + 'See ReactDOMServer.renderToString() for server rendering.') : undefined;
  ...
  Mixin: {
    ...
    mountChildren: function (nestedChildren, transaction, context) {
      ...
      for (var name in children) {
        if (children.hasOwnProperty(name)) {
          ...
          var rootID = this$1._rootNodeID + name;
      ...
      try {
        ...
        for (var name in prevChildren) {
          if (prevChildren.hasOwnProperty(name)) {
            this$1._unmountChild(prevChildren[name]);
          }
        }
    ...
    updateMarkup: function (nextMarkup) {
      ...
      try {
        ...
        for (var name in prevChildren) {
          if (prevChildren.hasOwnProperty(name)) {
            this$1._unmountChildByName(prevChildren[name], name);
          }
        }
    ...
    _updateChildren: function (nextNestedChildrenElements, transaction, context) {
      ...
      for (name in nextChildren) {
        ...
        if (prevChild === nextChild) {
          this$1.moveChild(prevChild, nextIndex, lastIndex);
          ...
          if (prevChild) {
            ...
            lastIndex = Math.max(prevChild._mountIndex, lastIndex);
            this$1._unmountChild(prevChild);
          }
          // The child must be instantiated before it's mounted.
          this$1._mountChildByNameAtIndex(nextChild, name, nextIndex, transaction, context);
        }
        nextIndex++;
      }
...
function createInternalComponent(element) {
  !genericComponentClass ? invariant(false, 'There is no registered component for the tag %s', element.type) : undefined;
...
var ReactOwner = {
  ...
  addComponentAsRefTo: function (component, ref, owner) {
    !ReactOwner.isValidOwner(owner) ? invariant(false, 'addComponentAsRefTo(...): Only a ReactOwner can have refs. You might ' + 'be adding a ref to a component that was not created inside a component\'s ' + '`render` method, or you have multiple copies of React loaded ' + '(details: https://fb.me/react-refs-must-have-owner).') : undefined;
...
function renderToString(element) {
  !ReactElement.isValidElement(element) ? invariant(false, 'renderToString(): You must pass a valid ReactElement.') : undefined;
...
function renderToStaticMarkup(element) {
  !ReactElement.isValidElement(element) ? invariant(false, 'renderToStaticMarkup(): You must pass a valid ReactElement.') : undefined;
...
function getInternalInstanceReadyForUpdate(publicInstance, callerName) {
  ...
  {
    warning(ReactCurrentOwner.current == null, '%s(...): Cannot update during an existing state transition ' + '(such as within `render`). Render methods should be a pure function ' + 'of props and state.', callerName);
  }
...
var ReactUpdateQueue = {
   ...
   */
  enqueueCallback: function (publicInstance, callback) {
    !(typeof callback === 'function') ? invariant(false, 'enqueueCallback(...): You called `setProps`, `replaceProps`, ' + '`setState`, `replaceState`, or `forceUpdate` with a callback that ' + 'isn\'t callable.') : undefined;
  ...
  enqueueSetPropsInternal: function (internalInstance, partialProps) {
    ...
    !topLevelWrapper ? invariant(false, 'setProps(...): You called `setProps` on a ' + 'component with a parent. This is an anti-pattern since props will ' + 'get reactively updated when rendered. Instead, change the owner\'s ' + '`render` method to pass the correct value as props to the component ' + 'where it is created.') : undefined;
  ...
  enqueueReplacePropsInternal: function (internalInstance, props) {
    var topLevelWrapper = internalInstance._topLevelWrapper;
    !topLevelWrapper ? invariant(false, 'replaceProps(...): You called `replaceProps` on a ' + 'component with a parent. This is an anti-pattern since props will ' + 'get reactively updated when rendered. Instead, change the owner\'s ' + '`render` method to pass the correct value as props to the component ' + 'where it is created.') : undefined;
...
function ensureInjected() {
  !(ReactUpdates.ReactReconcileTransaction && batchingStrategy) ? invariant(false, 'ReactUpdates: must inject a reconcile transaction class and batching ' + 'strategy') : undefined;
}
...
function runBatchedUpdates(transaction) {
  ...
  !(len === dirtyComponents.length) ? invariant(false, 'Expected flush transaction\'s stored dirty-components length (%s) to ' + 'match dirty-components array length (%s).', len, dirtyComponents.length) : undefined;
...
function asap(callback, context) {
  !batchingStrategy.isBatchingUpdates ? invariant(false, 'ReactUpdates.asap: Can\'t enqueue an asap callback in a context where' + 'updates are not being batched.') : undefined;
...
var ReactUpdatesInjection = {
  injectReconcileTransaction: function (ReconcileTransaction) {
    !ReconcileTransaction ? invariant(false, 'ReactUpdates: must provide a reconcile transaction class') : undefined;
    ReactUpdates.ReactReconcileTransaction = ReconcileTransaction;
  },

  injectBatchingStrategy: function (_batchingStrategy) {
    !_batchingStrategy ? invariant(false, 'ReactUpdates: must provide a batching strategy') : undefined;
    !(typeof _batchingStrategy.batchedUpdates === 'function') ? invariant(false, 'ReactUpdates: must provide a batchedUpdates() function') : undefined;
    !(typeof _batchingStrategy.isBatchingUpdates === 'boolean') ? invariant(false, 'ReactUpdates: must provide an isBatchingUpdates boolean attribute') : undefined;
...
var SimpleEventPlugin = {
  ...
  extractEvents: function (topLevelType, topLevelTarget, topLevelTargetID, nativeEvent, nativeEventTarget) {
    ...
    !EventConstructor ? invariant(false, 'SimpleEventPlugin: Unhandled event type, `%s`.', topLevelType) : undefined;
...
function SyntheticEvent(dispatchConfig, dispatchMarker, nativeEvent, nativeEventTarget) {
  ...
  for (var propName in Interface) {
    ...
    if (normalize) {
      this$1[propName] = normalize(nativeEvent);
    } else {
      this$1[propName] = nativeEvent[propName];
    }
  }
...
var Mixin = {
  ...
  perform: function (method, scope, a, b, c, d, e, f) {
    !!this.isInTransaction() ? invariant(false, 'Transaction.perform(...): Cannot initialize a transaction when there ' + 'is already an outstanding transaction.') : undefined;
    var errorThrown;
    var ret;
    try {
  ...
  initializeAll: function (startIndex) {
    ...
    for (var i = startIndex; i < transactionWrappers.length; i++) {
      ...
      try {
        ...
        this$1.wrapperInitData[i] = Transaction.OBSERVED_ERROR;
        this$1.wrapperInitData[i] = wrapper.initialize ? wrapper.initialize.call(this$1) : null;
      } finally {
        if (this$1.wrapperInitData[i] === Transaction.OBSERVED_ERROR) {
          // The initializer for wrapper i threw an error; initialize the
          // remaining wrappers but silence any exceptions from them to ensure
          // that the first error is the one to bubble up.
          try {
            this$1.initializeAll(i + 1);
          } catch (err) {}
        }
      }
    }
  ...
  closeAll: function (startIndex) {
    ...
    !this.isInTransaction() ? invariant(false, 'Transaction.closeAll(): Cannot close transaction when none are open.') : undefined;
    var transactionWrappers = this.transactionWrappers;
    for (var i = startIndex; i < transactionWrappers.length; i++) {
      var wrapper = transactionWrappers[i];
      var initData = this$1.wrapperInitData[i];
      var errorThrown;
      try {
        // Catching errors makes debugging more difficult, so we start with
        // errorThrown set to true before setting it to false after calling
        ...
        if (initData !== Transaction.OBSERVED_ERROR && wrapper.close) {
          wrapper.close.call(this$1, initData);
        }
        errorThrown = false;
      } finally {
        if (errorThrown) {
...
function findDOMNode(componentOrElement) {
  ...
  !(componentOrElement.render == null || typeof componentOrElement.render !== 'function') ? invariant(false, 'findDOMNode was called on an unmounted component.') : undefined;
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    var element = node;
    !(element && (typeof element.type === 'function' || typeof element.type === 'string')) ? invariant(false, 'Element type is invalid: expected a string (for built-in components) ' + 'or a class/function (for composite components) but got: %s.%s', element.type == null ? element.type : typeof element.type, getDeclarationErrorAddendum(element._owner)) : undefined;
...
function onlyChild(children) {
  !ReactElement.isValidElement(children) ? invariant(false, 'onlyChild must be passed a children with exactly one child.') : undefined;
  return children;
}
  ...
  validateDOMNesting = function (childTag, childInstance, ancestorInfo) {
    ...
    if (problematic) {
      ...
      if (invalidParent) {
        ...
        warning(false, 'validateDOMNesting(...): <%s> cannot appear as a child of <%s>. ' + 'See %s.%s', childTag, ancestorTag, ownerInfo, info);
      } else {
        warning(false, 'validateDOMNesting(...): <%s> cannot appear as a descendant of ' + '<%s>. See %s.', childTag, ancestorTag, ownerInfo);
      }
    }
  };
...
function createNodesFromMarkup(markup, handleScript) {
  var node = dummyNode;
  !!!dummyNode ? invariant(false, 'createNodesFromMarkup dummy not initialized') : undefined;
  ...
  if (scripts.length) {
    !handleScript ? invariant(false, 'createNodesFromMarkup(...): Unexpected <script> element rendered.') : undefined;
...
function getMarkupWrap(nodeName) {
  !!!dummyNode ? invariant(false, 'Markup wrapping node not initialized') : undefined;
...
var keyMirror = function (obj) {
  var ret = {};
  var key;
  !(obj instanceof Object && !Array.isArray(obj)) ? invariant(false, 'keyMirror(...): Argument must be an object.') : undefined;
  for (key in obj) {
...
function toArray(obj) {
  ...
  !(!Array.isArray(obj) && (typeof obj === 'object' || typeof obj === 'function')) ? invariant(false, 'toArray: Array-like object expected') : undefined;

  !(typeof length === 'number') ? invariant(false, 'toArray: Object needs a length property') : undefined;

  !(length === 0 || length - 1 in obj) ? invariant(false, 'toArray: Object should have keys for indices') : undefined;
...
var reactDom = createCommonjsModule(function (module, exports) {
/**
 * ReactDOM v0.14.2
 *
 * Copyright 2013-2015, Facebook, Inc.
